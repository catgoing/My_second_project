...
public class MyReviewCommand implements Command {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response)
		...
		if( null != vo ) {
			...
			//1. 검색한 데이터의 수를 구하기
			p.setTotalMember(MemberDAO.myReivewCount(id));
			p.setTotalPage();
...
//			System.out.println(">>끝페이지(endPage) : " + p.getEndPage());
			...
			// 현재 페이지에 표시할 데이터 조회
			List<ReviewVO> list = MemberDAO.myReview(map);
//			System.out.println("> 현재 페이지 글목록(list) : " + list);

			request.setAttribute("revlist", list); // 최종 데이터 리스트
